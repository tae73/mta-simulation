...
### 9.6 종합 매트릭스 (notebooks/part1/08_realworld_validation.ipynb 종합)
...
## 10. 실험 코드 감사 (Logic Audit)

리팩토링 과정에서 11개 실험 스크립트(`part1_simulation/experiments/01–11_*.py`)의
가설↔설정↔출력 정합성을 직접 검증했다. 수치 결과는 일절 변경하지 않았다(behavior 보존).

| Exp | 가설/설정 정합 | 발견 | 처리 |
|-----|----------------|------|------|
| 01 | OK | `run_all_methods(journeys, config)` 의 `config` 인자·`load_dgp_config(alpha_0=-5.625)` 가 dead (본문 미사용, journeys 만으로 결정) | **dead code 제거** — 정적으로 behavior-neutral 입증(미참조 인자/지역변수/import 삭제). LSTM/Transformer/CAMTA 가 비결정적이라 stochastic rerun 은 bit-identical 검증으로 부적합 → 정적 증명 채택 |
| 02–06 | OK | 가설·설정 docstring 명확, 결과 표와 일치 | 변경 없음 |
| 07 | OK | Exp 01 CSV 의 bias 컬럼에서 credit 재구성(재실행 불필요) — Exp 01 산출물 의존성. docstring 에 명시됨 | note만 (coupling 의도적·문서화됨) |
| 08 | OK | train/test split 은 **user-level 80/20, seed=42 고정**, "out-of-time 불가" 사유 docstring 명시 | 변경 없음 (초기 탐색의 "seed 불명" 우려는 사실무근) |
| 09 | 부분 | `revenue_lift_pct` 와 `allocation_mae` 가 동일 `derive_method_allocation` 파생 → 상관이 일부 정의상 기계적 | **docstring CAVEAT 추가** (해석 주의: 독립적 검증 아님; 코드/수치 불변) |
| 10 | OK | bootstrap 은 **user-level resample(`replace=True`) + 중복 user re-id**, `base_seed=42 + b` 시드 제어 | 변경 없음 (초기 탐색의 "resample 단위 불명" 우려는 사실무근) |
| 11 | OK | consensus = 18개 방법 mean rank, 가설과 일치 | 변경 없음 |

**감사 결론**: 코드 구조는 건전하다. 초기 탐색에서 제기된 다수 우려(Exp 09 circular
reasoning, Exp 10 resample 단위, Exp 08 split seed)는 직접 검증으로 **대부분 사실무근**으로
확인됐고, 실제 처리가 필요한 항목은 Exp 01 dead code(제거)와 Exp 09 해석 caveat(문서화) 2건뿐이다.
