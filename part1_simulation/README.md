...
### Poisson Survival 백본 위에 Incremental·Shapley 기여도와 path-level 분해를 통합한 인과 기반 Multi-Touch Attribution
...
> **time-to-event(생존) 데이터**로 보는 단일 Poisson GLM을 세우고 — 그 위에 **Incremental·Shapley
> 채널 기여도**와 **path-level(여정 단위) 분해**를 얹었다. *시뮬레이션 ground truth* 기준 18개 방법 중
> **채널 기여도 오차(MAE)가 가장 작으면서(0.016)**, 동일한 한 모델이 **채널 예산 · 여정 설계 · 전향적 인과
> 의사결정** 세 가지를 *하나의 효율성 항등식* 아래 답한다.
...
<p align="center">
  <img src="../assets/method_stack_ko.svg" alt="단일 Poisson GLM 위에 credit·path 층을 얹어 채널 예산·여정 설계·모집단 효과를 하나의 효율성 항등식으로 통일" width="820">
  <br><sub><b>그림 1.</b> 단일 fitted Poisson GLM(백본) 위에 credit 층과 path 층을 얹어,
  채널 예산 · 여정 설계 · 모집단 효과 세 관점을 <b>하나의 효율성 항등식(상대오차 0.00%)</b>으로 통일한다. 이것이 이 방법론의 핵심 주장이다.</sub>
</p>

- **무엇을 만들었나** — Shender et al.(2023)의 생존분석 백본과 Du et al.(2019)의 Incremental Shapley를
  *하나의 fitted GLM*으로 통합하고, 여기에 **path-level Incremental Shapley**(여정 단위 분해)와
...
이 프로젝트의 척추는 **하나의 fitted Poisson GLM 위에 세 개의 층을 쌓은 것**이다. 핵심은 *모델을 한 번만
적합하고*, 그 위에서 채널 기여도·여정 분해·모집단 효과를 **모두 동일한 게임**으로 도출한다는 점이다.
전체 구조는 위 **그림 1**(3층 스택 + 효율성 항등식)을 참조.

### 2.1 백본 — 왜 Poisson Survival인가
...
- **모델**: 각 터치포인트를 시간 구간으로 쪼갠 **interval-split Poisson GLM**.

  $$\log \lambda_i(t) \;=\; \alpha_0 \;+\; \sum_{j}\sum_{k} \beta_k \cdot x_{jk}\cdot f_{\text{channel}}(t-t_j)$$

  각 (터치포인트 × 시간구간)이 하나의 Poisson 관측이고 **offset = log(Δt)**. 우측 절단(right-censoring)이
  자연스럽게 처리된다.
- **데이터가 말하게 한다**: 채널별 5-bin 감쇠 곡선을 *학습*한다 — Paid Search는 ~1일 만에 식고, Display는
  ~14일까지 남는다. 임의의 lookback window를 가정하지 않는다.
...
- **왜 백본으로**: 단 하나의 GLM이 적합되므로, 이후 모든 기여도·여정·모집단 분해가 *같은 모델의 다른 질의*가 된다.

### 2.2 채널 기여도 — 왜 백본 위에 Incremental·Shapley를 얹나

적합된 intensity $\hat\lambda$는 아직 "채널 크레딧"이 아니다. 이를 채널별 기여로 분해하기 위해 **같은 GLM 위에서
...
두 연산자의 채널 **순위 합의도는 Spearman ρ = 0.929** — 합의가 높으면 보고 가능한 robust 신호, 크게 갈리면
시너지가 큰 채널이라는 진단 신호가 된다. (수식 유도: [`Methodology_05`](../docs/Methodology_05_Causal_Attribution_Frameworks.md) Eq. 13·25.)
...
- **같은 GLM, 다른 집계 단위**: 채널이 아니라 *여정 템플릿(ordered channel tuple)* 단위로
...
- **재현 가능한 것만 남긴다**: 전체 1,786개 고유 템플릿 중 ~98%가 1명짜리 우연한 긴 여정(count=1). `count ≥ 5`
  필터로 **35개 robust 템플릿**(전환자의 15.5% 커버)만 캠페인 후보로 남긴다. 남는 템플릿은 대부분 **짧고 빈번한
  2-step 여정**(median 길이 2)이다 — 필터 없이는 한 명의 우연한 긴 여정이 mean-Δ 1위로 올라와 실행 불가능해진다.
...
<p align="center">
  ...
  <br><sub><b>그림 2.</b> 전환에 조건부(converters-only)로 집계하면 collider bias가 들어온다. 과거 정산은 Conditional, 미래 예산은 Marginal.</sub>
</p>
...
<p align="center">
  ...
  <br><sub><b>그림 3.</b> ground-truth 대비 오차(MAE, ↓좋음) × 순위 일치도(Kendall τ, ↑좋음).
...
2. **인과 ≠ 자동 우승.** debiased 추정기(**DML 0.050, IPW 0.074**)는 이 DGP에서 **최고 규칙기반(Last Click
   0.038)을 못 이긴다** — 교란이 중간 수준이라 무거운 debiasing이 이득을 못 본다. 승부처는 *생존 + incremental
...
<p align="center">
  ...
  <br><sub><b>그림 4.</b> base 전환율이 오르면 Total Shapley는 Display·Social에서 0으로 붕괴하지만 Incremental은 안정.
...
이 방법론의 산출물은 *네 가지 의사결정*에 바로 연결된다. 핵심은 **하나의 fitted 모델이 네 질문에 모두 답한다**는 것.

<p align="center">
  <img src="../assets/industry_decision_map_ko.svg" alt="attribution 산출물 → 예산 재배분 · 여정 설계 · 의사결정 규칙 · 불확실성으로 이어지는 산업 활용 지도" width="860">
  <br><sub><b>그림 5.</b> 같은 GLM의 산출물이 예산·여정·전향 결정·불확실성 네 갈래로 연결된다.</sub>
</p>
...
<p align="center">
  ...
  <br><sub><b>그림 6.</b> Paid Search는 효과 1위지만 CPC가 비싸 최적 예산은 0.09%; 진짜 가치 동력은 Email(73.7%).
...
path-level Incremental Shapley(§2.3)는 *어떤 여정 패턴이 incremental 전환을 만드나*를 ranking한다. `count ≥ 5`로
거른 **35개 robust 템플릿**(대부분 짧고 빈번한 2-step 여정)은 "지금 작동하는" 여정 — 증폭(amplify) 우선순위가
된다. 채널 예산과 여정 설계가 **같은 효율성 항등식** 아래 정합하므로, 두 결정이 모순되지 않는다.
...
- **GT 없이 작동**: 실데이터엔 ground truth가 없다. 이 방법론은 AIC(스펙 선택) · OOS AUC(예측 sanity) ·
  내부 정합성(BackElim↔Shapley ρ, 효율성 항등식)으로 *스스로 검증*한다 — OOS AUC ~0.64는 reasonableness 게이트.
...
- **예측력은 modest.** OOS AUC가 ~0.64에서 방법 간 군집한다 — 예측검증은 방법을 *가르는* 도구가 아니라
  *기각하는*(너무 낮으면 신뢰 불가) reasonableness 게이트로만 쓴다.
...
# 2) 메인 인과 방법론 실행 (Survival/Poisson + Incremental Shapley + multi-path)
...
│   └── causal/
│       ├── survival_attribution.py   # ★ Poisson Survival 백본 (interval GLM)
...
각 실험의 데이터(CSV)는 [`results/part1/`](../results/part1/)에 있다 — 핵심 그림은 본문에 임베드(그림 3·4·6).
