...
방법론이 내놓는 두 산출물이자, 이 프로젝트에서 가장 중요한 두 실험이다.

**① Incremental vs Total — 왜 'incremental'이 필수인가 (실험 06).**
base 전환율을 ~3%에서 ~20%까지 올리며 **두 ground truth와 세 credit 연산자**를 한 그림에 겹쳤다. 두 ground truth는 묻는 질문이 다르다.
**GT_A**(intensity decomposition)는 *"전환에 누가 있었나"* 를 보는 진실이다. 전환자의 전환 강도를 채널별로 쪼갠 **사후 정산(conditional)** 의 관점이라, 강한 lower-funnel을 더 크게 본다.
**GT_B**(counterfactual)는 *"이 채널이 없었다면 무엇을 잃나"* 를 보는 진실이다. do(제거)의 순수 인과 lift이자 **A/B가 재는 전향(marginal)** 의 관점이라, upper-funnel의 인과 역할을 더 살린다.
두 진실은 채널 순위는 같지만 크기가 갈린다(§2.4; Paid Search는 GT_A 0.315 > GT_B 0.299, Display는 GT_B 0.084 > GT_A 0.061).
세 연산자 중 **Total Shapley**는 "광고가 없어도 일어났을" baseline 전환까지 크레딧에 넣기 때문에, base 전환율이 높아지면
**upper-funnel(Display·Social)을 0으로 무너뜨리고** Paid Search를 두 진실보다도 과대평가한다.
반면 **Incremental Shapley**는 baseline을 빼서 **두 진실 사이 밴드 안**에 머물러, base rate가 아무리 높아져도 안정적이다(BackElim은 last-touch에 크레딧이 쏠린다).
> **왜 중요한가**: 구독 갱신·브랜드처럼 자연 전환이 높은 비즈니스에서 Total Shapley를 쓰면 — 실제 가치 동력이 아닌 —
> **Paid Search에 예산을 과투자**하게 된다. incremental이 *옵션이 아니라 필수*인 이유다.

<p align="center">
  <img src="../results/part1/06_incremental_vs_total.png" alt="두 ground truth(GT_A·GT_B)와 세 연산자(Incremental·Total·BackElim) 채널 크레딧을 base rate별로 — Incremental은 진실 밴드, Total은 upper-funnel 0 붕괴" width="900">
  <br><sub><b>그림 2.</b> 같은 IPP 위 막대 다섯 개 — 두 ground truth(<b>GT_A</b> 회색·conditional, <b>GT_B</b> 초록·incremental, 둘 다 검은 테두리)와 세 연산자(Incremental·Total·BackElim)다.
  <b>Incremental(분홍)은 두 진실 밴드 안</b>에 머물지만 <b>Total(하늘)은 둘 모두에서 벗어난다</b>. Paid Search를 과대평가하고(high base 0.400 vs GT ~0.29) Display·Social은 <b>0으로 무너뜨린다</b>(GT 0.080·0.059인데 Total은 0.00).
  BackElim(주황)은 last-touch에 쏠린다. <i>실험 06 — 진짜 incremental credit이 왜 필요한지 보여준다.</i></sub>
</p>

**② Multi-path 기여도 — 채널이 아니라 '여정'을 평가 (path-level Incremental Shapley).**
같은 IPP를 채널이 아니라 *여정 템플릿(ordered channel tuple)* 단위로 분해한다. 1,786개 여정 중 `count ≥ 5`인
**35개 robust 템플릿**만 남겨 *재현 가능한* 캠페인 후보를 만든다. **total contribution** 상위(Email→Paid Search n=37 등)는
"지금 매출의 주축", **mean Δ** 상위(드문 3-step 여정)는 "의도 설계 후보"다.
> **왜 중요한가**: "어느 채널에 돈을 쓸까"(예산)와 "어떤 여정을 증폭할까"(캠페인 설계)는 보통 따로 분석하지만,
> 여기선 **같은 효율성 항등식** 아래 도출되어 두 결정이 모순되지 않는다 — 한 모델로 예산과 여정 설계를 동시에.

<p align="center">
  <img src="../results/part1/02_path_topk.png" alt="multi-path 기여도 — 35개 robust 여정 Top-K" width="900">
  <br><sub><b>그림 3.</b> path-level Incremental Shapley로 ranking한 35개 robust 여정. <b>(좌)</b> total contribution = 현 매출 주축(짧고 빈번한 2-step),
  <b>(우)</b> mean Δ = 의도 설계 후보(드물지만 강한 3-step). 색 = 여정 길이(touchpoint 수).</sub>
</p>
...
- **데이터가 말하게 한다**: 채널별 5-bin 감쇠 곡선을 *학습*한다 — Paid Search는 ~1일 만에 식고, Display는
  ~14일까지 남는다(아래 그림 4). 임의의 lookback window를 가정하지 않는다.
...
<p align="center">
  ...
  <br><sub><b>그림 4.</b> IPP가 데이터에서 <i>학습</i>한 채널별 감쇠(β, log-intensity 기여). Paid Search는 0–1일에 가장 강하다가
...
두 연산자의 채널 **순위 합의도는 Spearman ρ = 0.929**(아래 **그림 5**) — 합의가 높으면 보고 가능한 robust 신호, 크게
...
<p align="center">
  <img src="../results/part1/02_channel_credit.png" alt="BackElim vs Shapley 채널 기여도, Spearman ρ=0.929" width="680">
  <br><sub><b>그림 5.</b> 같은 IPP 위 두 credit 연산자(BackElim·Shapley)가 매긴 채널 기여도. 순위 합의 ρ=0.929 — robust한 신호.</sub>
</p>
...
붕괴**한다(위 **그림 2**, 실험 06: high base에서 Display는 GT 0.080·Incremental 0.088인데 Total은 0.00). Incremental Shapley는
baseline을 빼서 **광고가 만든 순증 lift만** 격리해 두 ground truth(GT_A·GT_B) 밴드 안에 머문다.
...
- **재현 가능한 것만 남긴다**: 전체 1,786개 고유 템플릿 중 ~98%가 1명짜리 우연한 긴 여정(count=1). `count ≥ 5`
  필터로 **35개 robust 템플릿**(전환자의 15.5% 커버)만 캠페인 후보로 남긴다(위 **그림 3**).
...
<p align="center">
  ...
  <br><sub><b>그림 6.</b> 전환에 조건부(converters-only)로 집계하면 collider bias가 들어온다. 과거 정산은 Conditional, 미래 예산은 Marginal.</sub>
</p>
...
<p align="center">
  ...
  <br><sub><b>그림 7.</b> ground-truth 대비 오차(MAE, ↓좋음) × 순위 일치도(Kendall τ, ↑좋음).
...
4. **최고 규칙기반은 의외로 경쟁적.** Last Click의 MAE(0.038)는 나쁘지 않다. 인과 계열의 진짜 우위는
   *순위·배분·안정성을 동시에* 만족하고 **해석 가능한 산출물**(감쇠 곡선, 시너지, incremental vs total)을 준다는 점이다.
   Total Shapley의 붕괴 거동은 위 **그림 2**(핵심 결과) 참조.
...
### 4.2 캠페인·여정 설계 — 35개 robust 템플릿에서 뽑는 전략 플레이북

path-level Incremental Shapley(§2.3, 위 그림 3)는 *어떤 여정 패턴이 incremental 전환을 만드는지* 순위를 매긴다.
전체 1,786개 고유 템플릿 중 `count ≥ 5`로 거른 **35개 robust 템플릿**(전환자의 15.5%)을 **두 렌즈**로 읽으면,
바로 실행할 수 있는 캠페인 플레이북이 된다. *예산이 아니라 여정 설계*의 질문에 답하는 셈이다.

| 렌즈 | 무엇 | top 템플릿 (실측) | 전략 액션 |
|---|---|---|---|
| **현 매출 주축** (total contribution = count × mean Δ) | "지금 스케일로 작동 중인 여정" | Email→Paid Search (n=37, 최대) · Direct→Paid Search (n=24) · Email→Direct (n=20) — 짧고 빈번한 2-step | **방어·증폭**: 예산 유지, 입찰 강화, 이탈 방지 |
| **의도 설계 후보** (mean Δ = per-user 순증) | "드물지만 1인당 임팩트가 큰 여정" | Direct→Paid Search→Paid Search (n=5, mean Δ 최고) · Email→Email→Paid Search (n=5) — 강한 3-step | **실험·확장**: A/B로 nurture 시퀀스 설계 후 스케일 |

> **왜 두 렌즈인가**: total contribution 상위는 *이미 매출을 만드는* 여정이라 **지키는** 것이 핵심이고, mean Δ 상위는
> *아직 드물지만 한 명당 lift가 큰* 여정이라 **키우는** 후보다. 그래서 예산은 mainstay를 방어하고, 실험 예산은 드물지만
> lift가 큰 여정으로 향한다. 채널 예산과 여정 설계가 **같은 효율성 항등식**에서 함께 나오므로(§2.3 channel↔path 쌍대성),
> 두 결정이 서로 어긋나지 않는다. *(출처: notebook 02 Cell 24 path-level 분해 — Email→Paid Search가 total contribution
> 1위, n=37. 값은 시뮬 ground truth 기준 illustrative.)*
...
### 4.5 왜 이 방법론으로 업그레이드했나 — 관리자 관점의 근거

관리자 입장에서 보면 이 업그레이드는 *모델 자랑*이 아니라 **예산 의사결정의 위험을 줄이는 변화**다.

1. **기존 attribution은 *틀린 예산*을 부른다.** Last Click·Linear·Total Shapley는 광고가 없어도 일어났을 baseline 전환까지
   크레딧에 넣어, high-intent 유저를 타고 가는 **Paid Search에 예산을 과배분**한다. 정작 효율 1위는 Email인데도 그렇다
   (§4.1, 효율 152.7 vs Paid Search 0.19). 규칙기반은 시간·비용 구조를 아예 무시한다.
2. **백본은 그대로 두고 credit 연산자만 incremental로 바꿔도 정확도가 크게 오른다.** 같은 IPP 위에서 BackElim을 Shapley로
   바꾼 것만으로 **채널 오차 MAE 2.9배·예산배분 4.4배 개선**된다(0.046→0.016, 0.083→0.019). 게다가 survival 백본은
   규칙기반이 놓치는 **우측 절단**("8시간 무전환 ≠ 효과 0")과 **채널별 시간 감쇠**를 정면으로 다룬다(§2.1).
3. **추정량이 결정과 맞아떨어진다.** Marginal G-computation이 복원하는 것은 **채널을 *do(제거)* 했을 때의 counterfactual,
   곧 A/B 테스트가 재려는 바로 그 estimand**다. 그래서 *오프라인 관측 추정을 나중에 A/B로 검증할 수 있고*, 미래 예산
   결정에도 들어맞는다. 두 view(Conditional/Marginal)의 채널 순위가 ρ=1.000으로 일치하므로 권고도 안정적이다.
4. **이해관계자가 신뢰할 수 있다.** 부트스트랩 변동성이 낮아(mean CV 0.13) 그대로 보고할 수 있고, 방법을 **causal —
   outcome-model only**(propensity 미보정)로 *정직하게 라벨링*해 과대주장을 피한다. 가정이 깨질 때를 대비한 다음
   업그레이드(DR/IPW 가중 survival, Tier 2)까지 밝혀둔다.

> 한 줄로: **"같은 데이터·같은 백본을 쓰되 결정에 맞는 estimand로 바꿨더니, 더 정확하고 더 안정적이며 A/B로 검증까지
> 되는 예산 권고가 나온다."** 이것이 incremental·survival 업그레이드를 *산업에서* 정당화하는 핵심이다.
...
| 06 | GT(A·B) vs Incremental·Total·BackElim | Incremental은 두 진실(GT_A·GT_B) 밴드 안; high base에서 Total은 upper-funnel 0 붕괴 | [`06_incremental_vs_total.csv`](../results/part1/06_incremental_vs_total.csv) |
...
그대로 보고한다. · <a href="README.en.md">English version →</a></sub>
