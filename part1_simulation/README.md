...
base 전환율을 ~3%에서 ~20%로 올리며 **Total Shapley**와 **Incremental Shapley**의 채널 크레딧을 비교했다.
Total Shapley는 "광고가 없어도 일어났을" baseline 전환까지 크레딧에 넣기 때문에, base 전환율이 높아지면
**upper-funnel 채널(Display·Social)의 크레딧이 0으로 붕괴**하고 lower-funnel(Paid Search)을 과대평가한다.
Incremental Shapley는 baseline을 빼 **광고가 만든 순증 lift만** 격리 → 어떤 base rate에서도 안정적이다.
...
<p align="center">
  <img src="../results/part1/06_incremental_vs_total.png" alt="Incremental vs Total Shapley, base-rate별 — Total은 upper-funnel에서 0으로 붕괴, Incremental은 안정" width="780">
  <br><sub><b>그림 2.</b> base 전환율이 오르면 Total Shapley(파랑)는 Display·Social에서 <b>0으로 붕괴</b>(high base: Display Total 0.00 vs Incremental 0.080)하지만
  Incremental(분홍)은 안정. Paid Search는 반대로 Total이 Incremental보다 과대(lower-funnel 과대크레딧). <i>실험 06.</i></sub>
</p>
...
붕괴**한다(위 **그림 2**, 실험 06: high base에서 Display Total = 0.00 vs Incremental = 0.080). Incremental Shapley는
...
path-level Incremental Shapley(§2.3, 위 그림 3)는 *어떤 여정 패턴이 incremental 전환을 만드나*를 ranking한다.
